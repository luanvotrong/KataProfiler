...
#define MAX_TEXTURES_NUMBER		(2048)
#define MAX_PROGRAMS_NUMBER		(2048)
#define MAX_SHADERS_NUMBER		(2048)
#define MAX_VBOS_NUMBER			(4096)

#define MAX_VERTEX_ATTRIBUTES_NUMBER		(256)
...
private:
	KPProgram*	getProgram(u32 id);
	KPProgram*	getFreeProgram();

	KPShader*	getShader(u32 id);
	KPShader*	getFreeShader();

	KPTexture*	getTexture(u32 id);
	KPTexture*	getFreeTexture();

	KPVbo*		getVbo(u32 id);
	KPVbo*		getFreeVbo();

	int calculateVertexAttribDataLength(KPProgram* prog, int vertexCount);

	void makeVertexAttribData(char* buffer, KPProgram* prog,
		bool isDrawElements,
			GLsizei countElements, GLenum type, const GLvoid* indices,
			GLint first, GLsizei countArrays);

	KPProgram* validate_glUniform1234fiv(const char* funcName, int comps, bool isFloat, bool isVector,
			GLint location, GLsizei count = 0);

	KPProgram* validate_glUniformMatrix(const char* funcName, int size, GLint location, GLsizei count, GLboolean transpose);

	void copyCurrentFboToTexture(const char* pixels, int width, int height);
	...
	KPVbo		m_pVbosList[MAX_VBOS_NUMBER];

	//
	KPVertexAttribData m_vertexAttributes[MAX_VERTEX_ATTRIBUTES_NUMBER];
