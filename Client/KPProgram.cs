	...
	public class KPVar
	{
		...
		public string toString()
		{
			if (string.IsNullOrEmpty(m_value))
				return "● " + m_location + ". " + gl2.getString(m_type) + " " + m_name;
			else
				return "● " + m_location + ". " + gl2.getString(m_type) + " " + m_name + " = " + m_value;
		}
	}

	public class KPProgram : KPGLObject
	{
		public const int MAX_ATTRIBUTES_NUMBER = 128;
		public const int MAX_UNIFORMS_NUMBER = 128;

		public KPProgram() : base()
		{
			clearData();
			init();
		}

		public KPProgram(uint id) : base(id)
		{
			clearData();
			init();
		}
		...
		private void init()
		{
			...
			for (int i = 0; i < MAX_ATTRIBUTES_NUMBER; i++) m_attributes[i] = new KPVar();
			m_uniforms = new KPVar[MAX_UNIFORMS_NUMBER];
			for (int i = 0; i < MAX_UNIFORMS_NUMBER; i++) m_uniforms[i] = new KPVar();
		}
		...
		private KPVar[] m_attributes;
		public KPVar[] Attributes { get { return m_attributes; } }

		private int m_uniformsCount;
		public int UniformsCount { get { return m_uniformsCount; } }

		private KPVar[] m_uniforms;
		public KPVar[] Uniforms { get { return m_uniforms; } }
		#endregion
