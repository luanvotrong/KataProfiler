...
namespace KataProfiler
{
	...
	public class KPMessage
	{
		...
		public void copyFrom(KPMessage msg)
		{
			reset();
		...
		public KPMessage createNewClone()
		{
			KPMessage clone = new KPMessage();
			clone.copyFrom(this);
			return clone;
		}
		...
		public string toString()
		{
			...
			switch (type)
			{
				...
				case KPMessageType.KMT_glActiveTexture:
				{
					...
					uint actualTexture = stream.readByte() + gl2.GL_TEXTURE0;
					stream.close();

					res = string.Format("glActiveTexture(texture = {0}, actualTexture = {1});",
						gl2.getString(texture),
						gl2.getString(actualTexture)
					);
				...
				case KPMessageType.KMT_glBindBuffer:
				{
					...
					res = string.Format("glBindBuffer(error = {0}, target = {1}, buffer = {2}, actualBoundBuffer = {3});",
						error, gl2.getString(target), buffer, actualBoundBuffer);
				...
				case KPMessageType.KMT_glBindFramebuffer:
				{
					...
					res = string.Format("glBindFramebuffer(target = {0}, framebuffer = {1});",
						gl2.getString(target), framebuffer);
				...
				case KPMessageType.KMT_glBindRenderbuffer:
				{
					...
					res = string.Format("glBindRenderbuffer(target = {0}, renderbuffer = {1});",
						gl2.getString(target), renderbuffer);
				...
				case KPMessageType.KMT_glBlendEquation:
				{
					...
					res = string.Format("glBlendEquation(mode = {0});", gl2.getString(mode));
				...
				case KPMessageType.KMT_glBlendEquationSeparate:
				{
					...
					res = string.Format("glBlendEquationSeparate(modeRGB = {0}, modeAlpha = {1});"
						, gl2.getString(modeRGB)
						, gl2.getString(modeAlpha));
				...
				case KPMessageType.KMT_glBlendFunc:
				{
					...
					res = string.Format("glBlendFunc(sfactor = {0}, dfactor = {1});"
						, gl2.getString(sfactor)
						, gl2.getString(dfactor));
				...
				case KPMessageType.KMT_glBlendFuncSeparate:
				{
					...
					res = string.Format("glBlendFuncSeparate(srcRGB = {0}, dstRGB = {1}, srcAlpha = {2}, dstAlpha = {3});"
						, gl2.getString(srcRGB, tmp1)
						, gl2.getString(dstRGB, tmp2)
						, gl2.getString(srcAlpha, tmp1)
						, gl2.getString(dstAlpha, tmp2)
					);
				...
				case KPMessageType.KMT_glClear:
				{
					...
					res = "[" + stream.readUInt() + "] " +
						"glClear(mask = " +
						gl2.getMask(stream.readUInt(), new string[] {
																"GL_DEPTH_BUFFER_BIT",
				...
				case KPMessageType.KMT_glCompressedTexSubImage2D:
				{
					...
					res = string.Format("glCompressedTexSubImage2D(target = {0}, level = {1}, " +
						"xoffset = {2}, yoffset = {3}, width = {4}, height = {5}, format = {6}, imageSize = {7}, data = 0x{8};)"
						, gl2.getString(target)
						, level
						, xoffset
						, yoffset
				...
				case KPMessageType.KMT_glCullFace:
				{
					...
					res = string.Format("glCullFace(mode = {0});", gl2.getString(mode));
				...
				{
					MyBinStream stream = new MyBinStream(m_pData);
					res = string.Format("glDepthFunc(func = {0});", gl2.getString( stream.readUInt() ));
					stream.close();
					break;
				}
				...
				{
					MyBinStream stream = new MyBinStream(m_pData);
					res = string.Format("glDepthMask(flag = {0});", gl2.getBoolean( stream.readByte() ));
					stream.close();
					break;
				}
				...
				{
					MyBinStream stream = new MyBinStream(m_pData);
					res = string.Format("glDisable(cap = {0});", gl2.getString( stream.readUInt() ));
					stream.close();
					break;
				}
				...
				case KPMessageType.KMT_glDrawArrays:
				{
					...
					res = "[" + stream.readUInt() + "] " + 
								"glDrawArrays(" +
								"mode = " + gl2.getString(stream.readUInt(), modes) +
				...
				case KPMessageType.KMT_glDrawElements:
				{
					...
					res = "[" + stream.readUInt() + "] " + 
								...
								"mode = " + gl2.getString(stream.readUInt(), modes) +
								", count = " + stream.readInt() +
								", type = " + gl2.getString(stream.readUInt()) +
				...
				{
					MyBinStream stream = new MyBinStream(m_pData);
					res = string.Format("glEnable(cap = {0});", gl2.getString( stream.readUInt() ));
					stream.close();
					break;
				}
				...
				{
					MyBinStream stream = new MyBinStream(m_pData);
					res = string.Format("glFrontFace(mode = {0});", gl2.getString(stream.readUInt()));
					stream.close();
					break;
				}
				...
				{
					MyBinStream stream = new MyBinStream(m_pData);
					res = string.Format("glGenerateMipmap(target = {0});", gl2.getString(stream.readUInt()));
					stream.close();
					break;
				}
				...
				case KPMessageType.KMT_glGetBooleanv:
				{
					...
					res = string.Format("glGetBooleanv(pname = {0}, params = {1});",
						gl2.getString( stream.readUInt() ),
						gl2.getBoolean( stream.readByte() )
					);
					stream.close();
					break;
				}
				...
				case KPMessageType.KMT_glGetError:
				{
					...
					MyBinStream stream = new MyBinStream(m_pData);
					res = string.Format("glGetError() = {0};", gl2.getString(stream.readUInt(), errors));
					stream.close();
					break;
				}
				...
				case KPMessageType.KMT_glGetTexParameterfv:
				{
					...
					res = string.Format("glGetTexParameterfv(target = {0}, pname = {1}, params = {2});",
						gl2.getString( stream.readUInt() ),
						gl2.getString( stream.readUInt() ),
						gl2.getString( (uint)stream.readFloat() )
					);
					break;
				}
				...
				case KPMessageType.KMT_glReadPixels:
				{
					...
					res = string.Format("glReadPixels(x = {0}, y = {1}, width = {2}, height = {3}, " +
						...
						gl2.getString( stream.readUInt() ), // format
						gl2.getString( stream.readUInt() ), // type
				...
				case KPMessageType.KMT_glRenderbufferStorage:
				{
					...
					res = string.Format("glRenderbufferStorage(target = {0}, internalformat = {1}, width = {2}, height = {3});",
						gl2.getString( stream.readUInt() ), // target
						gl2.getString( stream.readUInt() ), // internalformat
						stream.readInt(), // width
						stream.readInt() // height
					);
					stream.close();
				...
				{
					MyBinStream stream = new MyBinStream(m_pData);
					string face = gl2.getString(stream.readUInt());
					string fail = gl2.getString(stream.readUInt());
					string zfail = gl2.getString(stream.readUInt());
					string zpass = gl2.getString(stream.readUInt());
				...
				{
					MyBinStream stream = new MyBinStream(m_pData);
					string target = gl2.getString(stream.readUInt());
					string pname = gl2.getString(stream.readUInt());
					string param = gl2.getString( (uint)stream.readFloat() );
				...
				case KPMessageType.KMT_glTexParameterfv:
				{
					...
					string target = gl2.getString(stream.readUInt());
					string pname = gl2.getString(stream.readUInt());
					string _params = gl2.getString( (uint)stream.readFloat() );
				...
				case KPMessageType.KMT_glTexParameteri:
				{
					...
					string target = gl2.getString(stream.readUInt());
					string pname = gl2.getString(stream.readUInt());
					string param = gl2.getString(stream.readInt());
				...
				case KPMessageType.KMT_glTexParameteriv:
				{
					...
					string target = gl2.getString(stream.readUInt());
					string pname = gl2.getString(stream.readUInt());
					string _params = gl2.getString(stream.readInt());
				...
				case KPMessageType.KMT_glVertexAttribPointer:
				{
					...
					res = "glVertexAttribPointer(" +
						...
						", size = " + stream.readInt() +
						", type = " + gl2.getString( stream.readUInt() ) +
						", normalized = " + gl2.getBoolean(stream.readByte()) +
